...
trap_init(void)
{
	...
	//gate:  from interrupt descriptor table
	//istrap: istrap: 1 for a trap (= exception) gate, 0 for an interrupt gate. (doesn't talk about filling this out yet, could be important for lab 4?)
	...
	SETGATE(idt[T_BRKPT], 0, GD_KT, t_brkpt, 3);	//ring 3
	...
	SETGATE(idt[T_SYSCALL], 0, GD_KT, t_syscall, 3);	//ring 3
	...
	switch (tf->tf_trapno){
		...
		case T_SYSCALL:
		{
			int32_t ret = syscall(tf->tf_regs.reg_eax,
								  ...
								  tf->tf_regs.reg_edi,
								  tf->tf_regs.reg_esi);
			//Gets stored in eax for the return
